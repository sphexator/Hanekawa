            ...
            try
            {
                ...
                if (gameData.Combatstatus != 1)
                {
                    try
                    {
                        ...
                        if (response != null)
                        {
                            var eligible = ClassChangeService.ChangeEligibility(response.Content.ToString(), userData.Level);
                            if (eligible == 1)
                            {
                                GameDatabase.ChangeShipClass(user, response.Content);
                                await ReplyAsync($"{user.Username} successfully changed to {response.Content}").ConfigureAwait(false);
                            }
                            else
                            {
                                await ReplyAsync($"{user.Username}, you're not qualified for that class yet.").ConfigureAwait(false);
                            }
                        }
                        else
                        {
                ...
                else
                {
                    await ReplyAsync($"{user.Username} - You're currently in combat and cannot change class. \n" +
                        $"use **!flee** if you don't don't want to finish the fight.").ConfigureAwait(false);
                }
