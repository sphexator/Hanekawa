...
namespace Jibril.Modules.Welcome
{
    public class Welcome : InteractiveBase
    {
        ...
        public class WelcomeAdmin : InteractiveBase
        {
            ...
            public WelcomeAdmin(WelcomeService welcomeService)
            {
            ...
            public async Task AddWelcomeBanner(string url)
            {
                ...
                {
                    await ReplyAsync(null, false, new EmbedBuilder().Reply($"Okay then :awaueyes:", Color.Red.RawValue).Build());
                    return;
                }
                ...
                using (var db = new DbService())
                {
                    ...
                    await ReplyAsync(null, false, new EmbedBuilder().Reply($"Added banner to the collection!", Color.Green.RawValue).Build());
                }
            }
            ...
            public async Task SetWelcomeMessage([Remainder]string message)
            {
                using (var db = new DbService())
                {
                    ...
                    await db.SaveChangesAsync();
                    await ReplyAsync(null, false, new EmbedBuilder().Reply("Updated welcome message!", Color.Green.RawValue).Build());
                }
            }
            ...
            public async Task ToggleWelcome(ITextChannel channel = null)
            {
                using (var db = new DbService())
                {
                    ...
                    else if (cfg.WelcomeChannel.HasValue && channel != null)
                    {
                        ...
                        await ReplyAsync(null, false,
                            new EmbedBuilder().Reply($"Enabled welcome notifications in {channel.Mention}!", Color.Green.RawValue).Build());
                    }
                    else if (!cfg.WelcomeChannel.HasValue && channel == null)
                    {
