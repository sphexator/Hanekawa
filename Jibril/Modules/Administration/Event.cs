    ...
    public class Event : InteractiveBase
    {
        ...
        public async Task AddEventParticipantsAsync(IGuildUser user, int amount = 100){
            using(var db = new DbService()){
                ...
                await ReplyAsync(null, false, new EmbedBuilder().Reply($"Added {user.Mention} to the payout queue with an addition of {amount} special credit", Color.Green.RawValue).Build());
            }
        }
        ...
        public async Task RemoveEventParticipantAsync(IGuildUser user){
            using(var db = new DbService()){
                ...
                if(userdata == null){
                    await ReplyAsync(null, false, new EmbedBuilder().Reply($"Couldn't find {user.Mention} in the event payout queue!", Color.Red.RawValue).Build());
                    return;
                }
                db.EventPayouts.Remove(userdata);
                await db.SaveChangesAsync();
                await ReplyAsync(null, false, new EmbedBuilder().Reply($"Removed {user.Mention} with an amount of {userdata.Amount} from the event payout queue.", Color.Green.RawValue).Build());
            }
        }
        ...
        public async Task AdjustEventParticipant(IGuildUser user, int amount){
            using(var db = new DbService()){
                ...
                await db.SaveChangesAsync();
                await ReplyAsync(null, false, new EmbedBuilder().Reply($"Adjusted {user.Mention} special credit to {amount}", Color.Green.RawValue).Build());
            }
        }
        ...
        public async Task ListEventParticipantsAsync(){
            using(var db = new DbService()){
                ...
                if(users.Count <= 0){
                    await ReplyAsync(null, false, new EmbedBuilder().Reply($"No users in the queue.", Color.Red.RawValue).Build());
                    return;
                }
                string content = null;
                foreach(var x in users){
                    content += $"{Context.Guild.GetUser(x.UserId).Mention} - {x.Amount}\n";
                }
