...
namespace Hanekawa.AnimeSimulCast.Extensions
{
    ...
    {
        private static readonly Regex Season = new Regex(@"(S([1-9]{1,4}))",
            RegexOptions.IgnoreCase | RegexOptions.Compiled);

        private static readonly Regex Episode =
            new Regex(@"( - Episode ([\w]{1,4}))", RegexOptions.IgnoreCase | RegexOptions.Compiled);

        public static string GetSeason(this string content) => !Season.IsMatch(content) ? null : content.Remove(0, 1);
