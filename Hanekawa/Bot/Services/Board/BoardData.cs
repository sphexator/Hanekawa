...
namespace Hanekawa.Bot.Services.Board
{
    public partial class BoardService
    {
        ...
        private int GetReactionAmount(SocketGuild guild, SocketUserMessage msg)
        {
        ...
        private void IncreaseReactionAmount(SocketGuild guild, SocketUserMessage msg)
        {
            var messages = _reactionMessages.GetOrAdd(guild.Id, new MemoryCache(new MemoryCacheOptions()));
            var check = messages.TryGetValue(msg.Id, out var result);
            if (check)
        ...
        private void DecreaseReactionAmount(SocketGuild guild, SocketUserMessage msg)
        {
            var messages = _reactionMessages.GetOrAdd(guild.Id, new MemoryCache(new MemoryCacheOptions()));
            var check = messages.TryGetValue(msg.Id, out var result);
            if (!check) return;
