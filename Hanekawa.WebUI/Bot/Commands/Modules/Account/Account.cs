...
namespace Hanekawa.WebUI.Bot.Commands.Modules.Account
{
    ...
    public class Account : HanekawaCommandModule
    {
        ...
        public async Task<DiscordResponseCommandResult> RankAsync(IMember user = null)
        {
            ...
            user ??= Context.Author;
            var serverData = await db.GetOrCreateEntityAsync<global::Hanekawa.Entities.Account.Account>(user.GuildId, user.Id);
        ...
        {
            await using var db = Context.Scope.ServiceProvider.GetRequiredService<DbService>();
            var cdCheck = await db.GetOrCreateEntityAsync<global::Hanekawa.Entities.Account.Account>(Context.GuildId, Context.Author.Id);
