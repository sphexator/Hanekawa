...
using Hanekawa.WebUI.Bot.Service.Administration.Mute;
using Hanekawa.WebUI.Bot.Service.Administration.Warning;
using Hanekawa.WebUI.Bot.Service.Cache;
using Humanizer;
using Microsoft.Extensions.DependencyInjection;
using Qmmands;
using Range = Hanekawa.WebUI.Bot.Commands.TypeReaders.Range;

namespace Hanekawa.WebUI.Bot.Commands.Modules.Administration
{
    [Name("Administration")]
    ...
    public class Administration : HanekawaCommandModule
    {
        ...
        public async Task BanAsync(IMember user, [Remainder] string reason = "No reason applied")
        {
            ...
            if (!Context.Guild.HierarchyCheck(user))
            {
                await ReplyAndDeleteAsync(
                    new LocalMessage().Create("Cannot ban someone that's higher than me in hierarchy.",
            ...
            await ReplyAndDeleteAsync(new LocalMessage().Create(
        ...
        public async Task BanAsync(params Snowflake[] ids)
        {
            ...
            await ReplyAndDeleteAsync(
                new LocalMessage().Create($"Successfully banned {banned} accounts!", HanaBaseColor.Ok()),
        ...
        public async Task KickAsync(IMember user, [Remainder] string reason = "No reason applied")
        {
            ...
            {
                await ReplyAndDeleteAsync(
                    new LocalMessage().Create("Cannot kick someone that's higher than me in hierarchy.",
            ...
            await user.KickAsync(new DefaultRestRequestOptions
                {Reason = $"{Context.Author} ({Context.Author.Id}) reason: {reason}"});
            await ReplyAndDeleteAsync(new LocalMessage().Create(
        ...
        public async Task PruneAsync(int amount = 5, IMember user = null)
        {
            ...
            try
            {
                if (await Context.Channel.TryDeleteMessagesAsync(messages))
                    await ReplyAndDeleteAsync(
                        new LocalMessage().Create($"Deleted {amount} messages", Color.Green),
                        TimeSpan.FromSeconds(20));
                else
                    await ReplyAndDeleteAsync(new LocalMessage().Create(
            ...
            {
                await ReplyAndDeleteAsync(
                    new LocalMessage().Create("Couldn't delete messages, missing permissions?", Color.Red),
                    TimeSpan.FromSeconds(20));
            }
        }
        ...
        public async Task SoftBanAsync(IMember user)
        {
            ...
            if (!await _mute.Mute(user, Context.Author, "No Reason Specified (Soft ban)", db))
                await ReplyAndDeleteAsync(new LocalMessage().Create("Couldn't mute user.", Color.Red),
                    TimeSpan.FromSeconds(20));
        ...
        public async Task UnMuteAsync(IMember user, [Remainder] string reason = "No reason applied")
        {
            ...
            if (await _mute.UnMuteAsync(user, db))
                await ReplyAndDeleteAsync(new LocalMessage().Create($"Unmuted {user.Mention}", Color.Green),
                    TimeSpan.FromSeconds(20));
            else
                await ReplyAndDeleteAsync(new LocalMessage().Create(
        ...
        public async Task WarnAsync(IMember user, [Remainder] string reason = "No reason")
        {
            ...
            await ReplyAndDeleteAsync(new LocalMessage().Create($"Warned {user.Mention}", HanaBaseColor.Lime()),
                TimeSpan.FromSeconds(20));
        }
        ...
        public async Task<DiscordCommandResult> WarnLogAsync(IMember user, WarnLogType type = WarnLogType.Simple)
        {
            ...
            return Pages(pages.Select(x => new Page().AddEmbed(x)).ToList());
        }
        ...
        public async Task ReasonAsync(int id, [Remainder] string reason = "No reason applied")
        {
            ...
            await ReplyAndDeleteAsync(new LocalMessage().Create($"Updated mod log for {id}", HanaBaseColor.Lime()),
        ...
        [RequireBotGuildPermissions(Permission.ManageMessages)]
        [RequireAuthorGuildPermissions(Permission.ManageMessages)]
        public async Task ReasonAsync(TypeReaders.Range range, [Remainder] string reason = "No reason applied")
        {
            ...
            await ReplyAndDeleteAsync(
                new LocalMessage().Create($"Updated mod logs for entries between {range}", HanaBaseColor.Lime()),
                TimeSpan.FromSeconds(10));
        }
        ...
        private async Task UpdateMessage(ModLog modCase, DbService db, string reason)
        {
            ...
            {
                await ReplyAndDeleteAsync(
                    new LocalMessage().Create("Couldn't find the message. aborting...", HanaBaseColor.Bad()),
                    TimeSpan.FromSeconds(10));
                return;
            }

            var embed = LocalEmbed.FromEmbed(updMsg.Embeds[0]);
            if (embed == null)
            {
                await ReplyAndDeleteAsync(
                    new LocalMessage().Create("Couldn't find a embed to update...", HanaBaseColor.Bad()),
                    TimeSpan.FromSeconds(20));
                return;
            }
            ...
            await updMsg.ModifyAsync(m => m.Embeds = new[] {embed});
        }
    }
}