...
{
    public class CachedUserTypeParser : HanekawaTypeParser<CachedUser>
    {
        ...
        public override ValueTask<TypeParserResult<CachedUser>> ParseAsync(Parameter parameter, string value,
            HanekawaContext context, IServiceProvider provider)
        {
            if (context.Guild != null)
            {
                var memberParserResult = CachedMemberTypeParser.Instance.ParseAsync(parameter, value, context).Result;
            ...
            if (context.Message.Channel is CachedDmChannel dmChannel)
            {
